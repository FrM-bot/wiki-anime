...
const Details = ({ details, type } : IProps) => {
  ...
  // console.log(router.query.type, router.query.id, URL_CHARACTERS(router.query.type, router.query.id), URL_DETAILS({ type: router.query.type, id: router.query.id }))
  const { data: characters } : IResponseCharacters = useFetch(URL_CHARACTERS(type, details?.mal_id))
  // const { data: details, isLoading: isLoadingDetails, isError: isErrorDetails } : IResponseDetails = useFetch(URL_DETAILS({ type: router.query.type, id: router.query.id }))
  // const data = useFetchV2([URL_DETAILS({ type: router.query.type, id: router.query.id }), URL_CHARACTERS(router.query.type, router.query.id)])
  ...
  return (
    <Layout>
      {/* <LoadingComponent isError={isErrorDetails} isLoading={isLoadingDetails}> */}
        <>
        <div className='flex flex-wrap gap-2 mb-4'>
          <Card className='w-fit'>
            <h1 className='text-xl'>{details?.title}</h1>
          </Card>
          <ValidateAndRender dataToValidate={[details?.title_english]}>
...
export const getServerSideProps = async (context: { query: { type: string; id: number } }) => {
  ...
  try {
    ...
    // const characters: IResponseCharacters = await GET_CHARACTERS(type, id)
    // if (!details.data || !characters?.data) {
    //   throw new Error(`Failed to fetch posts, received status ${22}`)
    // }
    if (!details.data) {
      return {
        notFound: true
